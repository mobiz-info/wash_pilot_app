...
class _QuotationCreateScreenState extends State<QuotationCreateScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      try {
        final batteryRes = await ApiService.getBatteries(token);
        if (batteryRes['success'] == true) {
          _allBatteries = batteryRes['batteries'] ?? [];
        }
      } catch (_) {}
  ...
  // ── Battery Search Picker Bottom Sheet ───────────────────────────────────
  void _openBatterySearchPicker() {
    final searchCtrl = TextEditingController();
    List<dynamic> localList = List.from(_allBatteries);
                    ...
                    TextField(
                      ...
                      onChanged: (query) {
                        setModalState(() {
                          ...
                          localList = _allBatteries.where((b) {
                          ...
                          : ListView.separated(
                              ...
                              itemBuilder: (ctx, idx) {
                                ...
                                return ListTile(
                                  ...
                                  onTap: () {
                                    Navigator.pop(ctx);
                                    _showBatteryPriceEditDialog(item);
                                  },
                                );
                              },
                            ),
  ...
  void _showBatteryPriceEditDialog(Map<String, dynamic> battery) {
    ...
    final warrantyCtrl = TextEditingController(text: (battery['warranty_years'] ?? 1.0).toString());
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text('Default Price: $currencySymbol${defaultPrice.toStringAsFixed(2)}', style: GoogleFonts.inter(fontSize: 13.sp, fontWeight: FontWeight.w600, color: const Color(0xFF000080))),
                  ],
                ),
              ),
              const SizedBox(height: 14),
              Text('Warranty (Years):', style: GoogleFonts.inter(fontSize: 13.sp, fontWeight: FontWeight.bold)),
              ...
              const SizedBox(height: 14),
              Text('Battery Price ($currencySymbol) - Editable:', style: GoogleFonts.inter(fontSize: 13.sp, fontWeight: FontWeight.bold, color: const Color(0xFF000080))),
              const SizedBox(height: 6),
              TextField(
          ...
          actions: [
            ...
            ElevatedButton.icon(
              onPressed: () {
                ...
                final finalWarranty = double.tryParse(warrantyCtrl.text) ?? 1.0;
                Navigator.pop(dlgCtx);

                setState(() {
                  // Find or mock battery service
                  final batterySvc = _allServices.firstWhere(
                    (s) => (s['service_type_slug'] ?? '').toString().toLowerCase().contains('battery') || (s['name'] ?? '').toString().toLowerCase().contains('battery'),
                    orElse: () => {
                      'id': 'battery_${battery['id']}',
                      'name': 'Battery: ${battery['display_name']}',
                      'rate': finalPrice,
                    },
                  );
                  ...
                  _items.add(_QuotationItemRow(
                    service: batterySvc,
                    stockItem: {'id': battery['id'], 'item_name': battery['display_name']},
              ...
              label: Text(context.tr('Add Battery Item'), style: GoogleFonts.inter(fontWeight: FontWeight.bold)),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (_allBatteries.isNotEmpty) ...[
            InkWell(
              onTap: _openBatterySearchPicker,
              child: Container(
                ...
                child: Row(
                  children: [
                    const Icon(Icons.battery_charging_full, color: Color(0xFF000080), size: 22),
