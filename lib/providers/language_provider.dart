import 'dart:convert';
import 'package:flutter/material.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'package:provider/provider.dart';
import 'package:http/http.dart' as http;
import '../config/app_defaults.dart';

class LanguageProvider extends ChangeNotifier {
  // Google Cloud Translation API Key
  static const String _googleApiKey = 'AIzaSyD4Xeo3GNpP0ZhIMGeFub_o6pPwBmAHk5s';

  Locale _locale = const Locale('en');
  bool _isLoading = false;
  Map<String, String> _dynamicTranslations = {};
  final Set<String> _pendingTranslations = {};

  Locale get locale => _locale;
  bool get isLoading => _isLoading;

  LanguageProvider() {
    _loadLanguage();
  }

  // Load language from Shared Preferences and dynamic cache
  Future<void> _loadLanguage() async {
    final prefs = await SharedPreferences.getInstance();
    // Fall back to kDefaultLanguageCode (set by setup_country.dart) if no user preference saved
    final langCode = prefs.getString('language_code') ?? kDefaultLanguageCode;
    _locale = Locale(langCode);
    await _loadCachedTranslations(langCode);
    debugPrint('LanguageProvider initialized with locale: $langCode. Cached translations count: ${_dynamicTranslations.length}');
    notifyListeners();
  }

  // Save/Change language and download translations if not cached
  Future<void> setLanguage(String langCode) async {
    debugPrint('setLanguage called with langCode: $langCode');
    if (_locale.languageCode == langCode && _dynamicTranslations.isNotEmpty) {
      debugPrint('Language is already $langCode and cache is not empty. Skipping download.');
      return;
    }

    _isLoading = true;
    notifyListeners();

    try {
      final prefs = await SharedPreferences.getInstance();

      if (langCode != 'en') {
        final cachedJson = prefs.getString('cached_translations_$langCode');
        bool needsFetch = false;
        if (cachedJson == null) {
          debugPrint('No cached translations found for $langCode. Will download.');
          needsFetch = true;
        } else {
          try {
            final Map<String, dynamic> decoded = jsonDecode(cachedJson);
            if (decoded.isEmpty) {
              debugPrint('Cached translations for $langCode are empty. Will re-download.');
              needsFetch = true;
            } else {
              debugPrint('Found ${decoded.length} cached translations for $langCode.');
            }
          } catch (e) {
            debugPrint('Failed to parse cached translations for $langCode: $e. Will re-download.');
            needsFetch = true;
          }
        }

        if (needsFetch) {
          final Map<String, String> newTranslations = await _fetchTranslationsFromGoogle(langCode);
          if (newTranslations.isNotEmpty) {
            await prefs.setString('cached_translations_$langCode', jsonEncode(newTranslations));
            debugPrint('Successfully saved ${newTranslations.length} translations for $langCode to SharedPreferences.');
          } else {
            debugPrint('No translations returned from Google Translate for $langCode.');
          }
        }
      }

      _locale = Locale(langCode);
      await prefs.setString('language_code', langCode);
      await _loadCachedTranslations(langCode);
    } catch (e) {
      debugPrint('Error setting language / fetching Google translations: $e');
      // Fallback: still change language code so hardcoded translations work
      _locale = Locale(langCode);
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('language_code', langCode);
      await _loadCachedTranslations(langCode);
      rethrow;
    } finally {
      _isLoading = false;
      notifyListeners();
    }
  }

  // Load dynamic translations from local SharedPreferences cache
  Future<void> _loadCachedTranslations(String langCode) async {
    if (langCode == 'en') {
      _dynamicTranslations = {};
      return;
    }
    final prefs = await SharedPreferences.getInstance();
    final cachedJson = prefs.getString('cached_translations_$langCode');
    if (cachedJson != null) {
      try {
        final Map<String, dynamic> decoded = jsonDecode(cachedJson);
        _dynamicTranslations = decoded.map((key, value) => MapEntry(key, value.toString()));
        debugPrint('Loaded ${_dynamicTranslations.length} dynamic translations from cache for $langCode.');
      } catch (e) {
        debugPrint('Error decoding cached translations: $e');
        _dynamicTranslations = {};
      }
    } else {
      debugPrint('No SharedPreferences entry for cached_translations_$langCode');
      _dynamicTranslations = {};
    }
  }

  // Fetch all translations for a given language code using Google Translate API in chunks
  Future<Map<String, String>> _fetchTranslationsFromGoogle(String langCode) async {
    debugPrint('Starting fetch from Google Translate for language: $langCode');
    if (_googleApiKey.isEmpty || _googleApiKey == 'YOUR_GOOGLE_TRANSLATION_API_KEY') {
      debugPrint('Google Translation API Key is missing or default. Falling back to local maps.');
      return {};
    }

    final englishMap = _translations['en'];
    if (englishMap == null || englishMap.isEmpty) {
      debugPrint('English translations map is empty or null.');
      return {};
    }

    final keys = englishMap.keys.toList();
    final Map<String, String> result = {};

    // Use chunks of 40 to avoid hitting Google Translate request payload or URI size limitations
    const int chunkSize = 40;
    debugPrint('Translating ${keys.length} keys in chunks of $chunkSize...');

    for (int i = 0; i < keys.length; i += chunkSize) {
      final chunkKeys = keys.sublist(i, i + chunkSize > keys.length ? keys.length : i + chunkSize);
      final chunkValues = chunkKeys.map((k) => englishMap[k]!).toList();

      final url = Uri.parse('https://translation.googleapis.com/language/translate/v2?key=$_googleApiKey');
      
      debugPrint('Posting chunk ${i ~/ chunkSize + 1} (${chunkKeys.length} items) to Google Translate...');
      final response = await http.post(
        url,
        headers: {'Content-Type': 'application/json; charset=utf-8'},
        body: jsonEncode({
          'q': chunkValues,
          'target': langCode,
          'source': 'en',
          'format': 'text',
        }),
      );

      debugPrint('Google Translate response status for chunk ${i ~/ chunkSize + 1}: ${response.statusCode}');
      if (response.statusCode == 200) {
        final Map<String, dynamic> data = jsonDecode(response.body);
        final List<dynamic> translations = data['data']['translations'];
        debugPrint('Received ${translations.length} translations for chunk.');
        
        for (int j = 0; j < chunkKeys.length; j++) {
          if (j < translations.length) {
            result[chunkKeys[j]] = translations[j]['translatedText']?.toString() ?? chunkValues[j];
          } else {
            result[chunkKeys[j]] = chunkValues[j];
          }
        }
      } else {
        debugPrint('Google Translate error response body: ${response.body}');
        throw Exception('Google Translate API error: ${response.statusCode} - ${response.body}');
      }
    }

    debugPrint('Finished fetching all translations from Google. Total items translated: ${result.length}');
    return result;
  }

  // Translation helper (Checks dynamic cache first, then static fallbacks, then English)
  String translate(String key) {
    final currentLang = _locale.languageCode;
    
    // 1. If English, return English value directly
    if (currentLang == 'en') {
      return _translations['en']?[key] ?? key;
    }

    // 2. Try dynamically translated and cached values from Google API
    if (_dynamicTranslations.containsKey(key)) {
      return _dynamicTranslations[key]!;
    }

    // 3. Try hardcoded static fallback translations
    if (_translations.containsKey(currentLang) &&
        _translations[currentLang]!.containsKey(key)) {
      return _translations[currentLang]![key]!;
    }

    // 4. Try English master dictionary as a last resort
    if (_translations['en']!.containsKey(key)) {
      return _translations['en']!.containsKey(key) ? _translations['en']![key]! : key;
    }

    return key;
  }

  // Dynamic on-demand translation helper for hardcoded English texts
  String translateDynamic(String text) {
    if (text.isEmpty) return text;
    
    final currentLang = _locale.languageCode;
    if (currentLang == 'en') {
      return text;
    }

    // 1. If the text itself matches a key in our translations map, translate it using standard method
    if (_translations['en']!.containsKey(text)) {
      return translate(text);
    }
    
    // Also check lowercase version of the text
    final lowerKey = text.toLowerCase().replaceAll(' ', '_');
    if (_translations['en']!.containsKey(lowerKey)) {
      return translate(lowerKey);
    }

    // 2. Check if we already have it in our dynamic cache
    if (_dynamicTranslations.containsKey(text)) {
      return _dynamicTranslations[text]!;
    }

    // 3. Trigger asynchronous background translation
    if (!_pendingTranslations.contains(text)) {
      _pendingTranslations.add(text);
      _fetchAndCacheSingleText(text, currentLang);
    }

    return text;
  }

  // Fetch translation for a single text asynchronously in the background and notify listeners
  Future<void> _fetchAndCacheSingleText(String text, String langCode) async {
    try {
      if (_googleApiKey.isEmpty || _googleApiKey == 'YOUR_GOOGLE_TRANSLATION_API_KEY') {
        _pendingTranslations.remove(text);
        return;
      }

      final url = Uri.parse('https://translation.googleapis.com/language/translate/v2?key=$_googleApiKey');
      
      final response = await http.post(
        url,
        headers: {'Content-Type': 'application/json; charset=utf-8'},
        body: jsonEncode({
          'q': [text],
          'target': langCode,
          'source': 'en',
          'format': 'text',
        }),
      );

      if (response.statusCode == 200) {
        final Map<String, dynamic> data = jsonDecode(response.body);
        final List<dynamic> translations = data['data']['translations'];
        if (translations.isNotEmpty) {
          final translatedText = translations[0]['translatedText']?.toString() ?? text;
          
          // Save in memory
          _dynamicTranslations[text] = translatedText;
          
          // Save in SharedPreferences
          final prefs = await SharedPreferences.getInstance();
          await prefs.setString('cached_translations_$langCode', jsonEncode(_dynamicTranslations));
          debugPrint('On-demand translation cached: "$text" -> "$translatedText" ($langCode)');
        }
      }
    } catch (e) {
      debugPrint('Error in dynamic translation for "$text": $e');
    } finally {
      _pendingTranslations.remove(text);
      notifyListeners();
    }
  }

  static const Map<String, Map<String, String>> _translations = {
    'en': {
      'dashboard': 'Dashboard',
      'menu': 'Menu',
      'new_job': 'New Job',
      'book_now': 'Book Now',
      'bookings': 'Bookings',
      'bill': 'Bill',
      'outstanding': 'Outstanding',
      'receipt': 'Receipt',
      'customers': 'Customers',
      'schemes': 'Schemes',
      'reports': 'Reports',
      'vehicle': 'Vehicle',
      'notifications': 'Notifications',
      'language': 'Language',
      'complaints': 'Complaints',
      'logout': 'Logout',
      'welcome_back': 'Welcome Back',
      'add_extras': 'Add Extras',
      'services_subtotal': 'Services Subtotal',
      'scheme_discount': 'Scheme Discount',
      'manual_discount': 'Manual Discount',
      'bill_summary': 'Bill Summary',
      'amount_collected': 'Amount Collected',
      'taxes': 'Taxes',
      'quick_menu': 'Quick Menu',
      'login': 'Login',
      'username_or_email': 'Username or Email',
      'enter_username_or_email': 'Enter username or email',
      'password': 'Password',
      'enter_password': 'Enter password',
      'remember_me': 'Remember me',
      'forgot_password': 'Forgot Password?',
      'sign_in': 'Sign In',
      'logout_confirm': 'Are you sure you want to logout?',
      'cancel': 'Cancel',
      'branch_admin': 'Branch Admin',
      'company_admin': 'Company Admin',
      'recent_invoices': 'Recent Invoices',
      'select_language': 'Select Language',
      'admin_login': 'Admin Login',
      'username': 'Username',
      'please_enter_credentials': 'Please enter username and password',
      'todays_summary': "Today's Summary",
      'overview': 'Overview',
      'todays_recent_jobs': "Today's Recent Jobs",
      'revenue': 'Revenue',
      'collected': 'Collected',
      'jobs': 'Jobs',
      'due': 'Due',
      'paid': 'Paid',
      'invoices_count': 'invoices',
      'exit_app': 'Exit App',
      'exit_confirm': 'Do you want to close the app?',
      'exit': 'Exit',
      'bills': 'Bills',
      'vehicle_search': 'Vehicle Search',
      'not_authenticated': 'Not authenticated',
      'network_error': 'Network error',
      'failed_to_load': 'Failed to load',
      'retry': 'Retry',
      'save': 'Save',
      'saving': 'Saving...',
      'delete': 'Delete',
      'add': 'Add',
      'create': 'Create',
      'date': 'Date',
      'start': 'Start',
      'end': 'End',
      'status': 'Status',
      'description': 'Description',
      'name': 'Name',
      'phone': 'Phone',
      'email': 'Email',
      'address': 'Address',
      'actions': 'Actions',
      'total': 'Total',
      'balance': 'Balance',
      'amount': 'Amount',
      'all': 'All',
      'available_schemes': 'Available Schemes',
      'no_schemes_available': 'No schemes available',
      'add_scheme': 'Add Scheme',
      'scheme_name': 'Scheme Name',
      'scheme_type': 'Scheme Type',
      'paid_visits': 'Paid Visits',
      'free_visits': 'Free Visits',
      'discount_percentage': 'Discount Percentage',
      'voucher_no': 'Voucher No',
      'add_voucher': 'Add Voucher',
      'services': 'Services',
      'customer_types': 'Customer Types',
      'vehicle_types': 'Vehicle Types',
      'save_scheme': 'Save Scheme',
      'scheme_created': 'Scheme created',
      'enter_scheme_details_error': 'Enter scheme name and type',
      'booking_details': 'Booking Details',
      'customer_name': 'Customer Name',
      'phone_number': 'Phone Number',
      'vehicle_number': 'Vehicle Number',
      'select_service': 'Select Service',
      'select_date': 'Select Date',
      'time_slot': 'Time Slot',
      'booking_created': 'Booking created',
      'booking_failed': 'Booking failed',
      'bookings_list': 'Bookings List',
      'no_bookings_found': 'No bookings found',
      'receipts': 'Receipts',
      'no_receipts_found': 'No receipts found',
      'collection': 'Collection',
      'collect_payment': 'Collect Payment',
      'payment_collected': 'Payment collected',
      'add_complaint': 'Add Complaint',
      'subject': 'Subject',
      'complaint_created': 'Complaint created',
      'no_complaints_found': 'No complaints found',
      'add_new_customer': 'Add New Customer',
      'edit_customer': 'Edit Customer',
      'customer_created': 'Customer created',
      'customer_updated': 'Customer updated',
      'no_customers_found': 'No customers found',
      'create_invoice': 'Create Invoice',
      'invoice_details': 'Invoice Details',
      'invoice_created': 'Invoice created',
      'invoice_number': 'Invoice Number',
      'subtotal': 'Subtotal',
      'tax': 'Tax',
      'grand_total': 'Grand Total',
      'select_vehicle': 'Select Vehicle',
      'select_scheme': 'Select Scheme',
      'job_details': 'Job Details',
      'revenue_report': 'Revenue Report',
      'jobs_report': 'Jobs Report',
      'sales_summary': 'Sales Summary',
      'search_by_vehicle_number': 'Search by Vehicle Number',
      'vehicle_details': 'Vehicle Details',
      'msg_welcome': 'Hi {customer} Welcome to Our {branch}.Your vehicle {vehicle} has arrived safely and is in expert hands.We will keep you posted!',
      'msg_ready_alert': 'Hi {customer} Great news! Your vehicle {vehicle} is ready for pickup. Please collect at your earliest convenience.\n{branch} Support team.',
      'msg_thanks': 'Dear {customer} 🙏 Thank you for trusting us with your vehicle {vehicle}. We hope you had a great experience. Looking forward to seeing you again!\n{branch} support team.',
    },
    'fr': {
      'dashboard': 'Tableau de bord',
      'menu': 'Menu',
      'new_job': 'Nouveau travail',
      'book_now': 'Réserver maintenant',
      'bookings': 'Réservations',
      'bill': 'Facture',
      'outstanding': 'En cours',
      'receipt': 'Reçu',
      'customers': 'Clients',
      'schemes': 'Programmes',
      'reports': 'Rapports',
      'vehicle': 'Véhicule',
      'notifications': 'Notifications',
      'language': 'Langue',
      'complaints': 'Réclamations',
      'logout': 'Déconnexion',
      'welcome_back': 'Bienvenue',
      'add_extras': 'Ajouter des extras',
      'services_subtotal': 'Sous-total des services',
      'scheme_discount': 'Remise du programme',
      'manual_discount': 'Remise manuelle',
      'bill_summary': 'Résumé de la facture',
      'amount_collected': 'Montant perçu',
      'taxes': 'Taxes',
      'quick_menu': 'Menu rapide',
      'login': 'Connexion',
      'username_or_email': 'Nom d\'utilisateur ou e-mail',
      'enter_username_or_email': 'Entrez le nom d\'utilisateur ou l\'e-mail',
      'password': 'Mot de passe',
      'enter_password': 'Entrez le mot de passe',
      'remember_me': 'Se souvenir de moi',
      'forgot_password': 'Mot de passe oublié ?',
      'sign_in': 'Se connecter',
      'logout_confirm': 'Êtes-vous sûr de vouloir vous déconnecter ?',
      'cancel': 'Annuler',
      'branch_admin': 'Admin de succursale',
      'company_admin': 'Admin d\'entreprise',
      'recent_invoices': 'Factures récentes',
      'select_language': 'Choisir la langue',
      'admin_login': 'Connexion Admin',
      'username': 'Nom d\'utilisateur',
      'please_enter_credentials': 'Veuillez entrer le nom d\'utilisateur et le mot de passe',
      'todays_summary': 'Résumé d\'aujourd\'hui',
      'overview': 'Aperçu',
      'todays_recent_jobs': 'Travaux récents d\'aujourd\'hui',
      'revenue': 'Revenu',
      'collected': 'Perçu',
      'jobs': 'Travaux',
      'due': 'Dû',
      'paid': 'Payé',
      'invoices_count': 'factures',
      'exit_app': 'Quitter l\'application',
      'exit_confirm': 'Voulez-vous fermer l\'application ?',
      'exit': 'Quitter',
      'bills': 'Factures',
      'vehicle_search': 'Recherche de véhicule',
      'not_authenticated': 'Non authentifié',
      'network_error': 'Erreur réseau',
      'failed_to_load': 'Échec du chargement',
      'retry': 'Réessayer',
      'save': 'Enregistrer',
      'saving': 'Enregistrement...',
      'delete': 'Supprimer',
      'add': 'Ajouter',
      'create': 'Créer',
      'date': 'Date',
      'start': 'Début',
      'end': 'Fin',
      'status': 'Statut',
      'description': 'Description',
      'name': 'Nom',
      'phone': 'Téléphone',
      'email': 'E-mail',
      'address': 'Adresse',
      'actions': 'Actions',
      'total': 'Total',
      'balance': 'Solde',
      'amount': 'Montant',
      'all': 'Tout',
      'available_schemes': 'Programmes disponibles',
      'no_schemes_available': 'Aucun programme disponible',
      'add_scheme': 'Ajouter un programme',
      'scheme_name': 'Nom du programme',
      'scheme_type': 'Type de programme',
      'paid_visits': 'Visites payées',
      'free_visits': 'Visites gratuites',
      'discount_percentage': 'Pourcentage de remise',
      'voucher_no': 'N° de coupon',
      'add_voucher': 'Ajouter un coupon',
      'services': 'Services',
      'customer_types': 'Types de clients',
      'vehicle_types': 'Types de véhicules',
      'save_scheme': 'Enregistrer le programme',
      'scheme_created': 'Programme créé',
      'enter_scheme_details_error': 'Entrez le nom et le type du programme',
      'booking_details': 'Détails de la réservation',
      'customer_name': 'Nom du client',
      'phone_number': 'Numéro de téléphone',
      'vehicle_number': 'Numéro de véhicule',
      'select_service': 'Sélectionner un service',
      'select_date': 'Sélectionner une date',
      'time_slot': 'Créneau horaire',
      'booking_created': 'Réservation créée',
      'booking_failed': 'Échec de la réservation',
      'bookings_list': 'Liste des réservations',
      'no_bookings_found': 'Aucune réservation trouvée',
      'receipts': 'Reçus',
      'no_receipts_found': 'Aucun reçu trouvé',
      'collection': 'Encaissement',
      'collect_payment': 'Encaisser le paiement',
      'payment_collected': 'Paiement encaissé',
      'add_complaint': 'Ajouter une réclamation',
      'subject': 'Sujet',
      'complaint_created': 'Réclamation créée',
      'no_complaints_found': 'Aucune réclamation trouvée',
      'add_new_customer': 'Ajouter un nouveau client',
      'edit_customer': 'Modifier le client',
      'customer_created': 'Client créé',
      'customer_updated': 'Client mis à jour',
      'no_customers_found': 'Aucun client trouvé',
      'create_invoice': 'Créer une facture',
      'invoice_details': 'Détails de la facture',
      'invoice_created': 'Facture créée',
      'invoice_number': 'Numéro de facture',
      'subtotal': 'Sous-total',
      'tax': 'Taxe',
      'grand_total': 'Total général',
      'select_vehicle': 'Sélectionner un véhicule',
      'select_scheme': 'Sélectionner un programme',
      'job_details': 'Détails du travail',
      'revenue_report': 'Rapport de revenus',
      'jobs_report': 'Rapport de travaux',
      'sales_summary': 'Résumé des ventes',
      'search_by_vehicle_number': 'Rechercher par numéro de véhicule',
      'vehicle_details': 'Détails du véhicule',
      'INVOICE': 'FACTURE',
      'Invoice': 'Facture',
      'BILLED TO': 'FACTURÉ À',
      'VEHICLE': 'VÉHICULE',
      'SERVICES': 'SERVICES',
      'Service': 'Service',
      'Rate': 'Tarif',
      'Discount': 'Remise',
      'Line Total': 'Total Ligne',
      'Subtotal': 'Sous-total',
      'Total Discount': 'Remise Totale',
      'Tax': 'Taxe',
      'TOTAL': 'TOTAL',
      'Credit Invoice': 'Facture à Crédit',
      'Cash Invoice': 'Facture au Comptant',
      'Phone': 'Téléphone',
      'Email': 'E-mail',
      'GSTIN': 'GSTIN',
      'Thank you for choosing': 'Merci d\'avoir choisi',
      'Powered by': 'Propulsé par',
      'Dear': 'Cher(ère)',
      'Your invoice': 'Votre facture',
      'has been generated successfully at': 'a été générée avec succès chez',
      'Invoice Details': 'Détails de la facture',
      'Paid': 'Payé',
      'Balance': 'Solde',
      'Oil:': 'Huile:',
      'Litres:': 'Litres:',
      'Filter:': 'Filtre:',
      'Odometer:': 'Odomètre:',
      'Next Change:': 'Prochain changement:',
      'Share Invoice': 'Partager la facture',
      'Share PDF Document': 'Partager le document PDF',
      'QUOTATION': 'DEVIS',
      'QUOTATION FOR': 'DEVIS POUR',
      'VEHICLE DETAILS': 'DÉTAILS DU VÉHICULE',
      'SERVICES & STOCK ITEMS': 'SERVICES & ARTICLES EN STOCK',
      'Service / Item': 'Service / Article',
      'Warranty': 'Garantie',
      'EXTRAS': 'EXTRAS',
      'Item Name': 'Nom de l\'article',
      'Price': 'Prix',
      'ADDITIONAL SERVICES / NOTES:': 'SERVICES SUPPLÉMENTAIRES / NOTES:',
      'GRAND TOTAL': 'TOTAL GÉNÉRAL',
      'msg_welcome': 'Bonjour {customer}! 👋 Bienvenue dans notre centre de service. Votre véhicule {vehicle} est arrivé en toute sécurité et est entre de bonnes mains. Nous vous tiendrons informé!',
      'msg_ready_alert': 'Bonjour {customer}! 🎉 Bonne nouvelle! Votre véhicule {vehicle} est prêt pour la récupération. Veuillez le récupérer dès que possible.',
      'msg_thanks': 'Cher {customer}, 🙏 Merci de nous avoir confié votre véhicule {vehicle}. Nous espérons que vous avez eu une excellente expérience. À bientôt!',
    },
    'hi': {
      'dashboard': 'डैशबोर्ड',
      'menu': 'मेनू',
      'new_job': 'नया कार्य',
      'book_now': 'अभी बुक करें',
      'bookings': 'बुकिंग',
      'bill': 'बिल',
      'outstanding': 'बकाया',
      'receipt': 'रसीद',
      'customers': 'ग्राहक',
      'schemes': 'योजनाएं',
      'reports': 'रिपोर्ट',
      'vehicle': 'वाहन',
      'notifications': 'सूचनाएं',
      'language': 'भाषा',
      'complaints': 'शिकायतें',
      'logout': 'लॉग आउट',
      'welcome_back': 'आपका स्वागत है',
      'add_extras': 'अतिरिक्त जोड़ें',
      'services_subtotal': 'सेवाओं का योग',
      'scheme_discount': 'योजना छूट',
      'manual_discount': 'मैन्युअल छूट',
      'bill_summary': 'बिल सारांश',
      'amount_collected': 'एकत्रित राशि',
      'taxes': 'कर',
      'quick_menu': 'त्वरित मेनू',
      'login': 'लॉगिन',
      'username_or_email': 'उपयोगकर्ता नाम या ईमेल',
      'enter_username_or_email': 'उपयोगकर्ता नाम या ईमेल दर्ज करें',
      'password': 'पासवर्ड',
      'enter_password': 'पासवर्ड दर्ज करें',
      'remember_me': 'मुझे याद रखें',
      'forgot_password': 'पासवर्ड भूल गए?',
      'sign_in': 'साइन इन करें',
      'logout_confirm': 'क्या आप वाकई लॉग आउट करना चाहते हैं?',
      'cancel': 'रद्द करें',
      'branch_admin': 'शाखा व्यवस्थापक',
      'company_admin': 'कंपनी व्यवस्थापक',
      'recent_invoices': 'हाल के इनवॉइस',
      'select_language': 'भाषा चुनें',
      'admin_login': 'व्यवस्थापक लॉगिन',
      'username': 'उपयोगकर्ता नाम',
      'please_enter_credentials': 'कृपया उपयोगकर्ता नाम और पासवर्ड दर्ज करें',
      'todays_summary': 'आज का सारांश',
      'overview': 'अवलोकन',
      'todays_recent_jobs': 'आज के हालिया कार्य',
      'revenue': 'राजस्व',
      'collected': 'एकत्रित',
      'jobs': 'कार्य',
      'due': 'देय',
      'paid': 'भुगतान किया गया',
      'invoices_count': 'इनवॉइस',
      'exit_app': 'ऐप से बाहर निकलें',
      'exit_confirm': 'क्या आप ऐप बंद करना चाहते हैं?',
      'exit': 'बाहर निकलें',
      'bills': 'बिल',
      'vehicle_search': 'वाहन खोज',
      'not_authenticated': 'प्रमाणित नहीं है',
      'network_error': 'नेटवर्क त्रुटि',
      'failed_to_load': 'लोड करने में विफल',
      'retry': 'पुनः प्रयास करें',
      'save': 'सहेजें',
      'saving': 'सहेज रहा है...',
      'delete': 'हटाएं',
      'add': 'जोड़ें',
      'create': 'बनाएं',
      'date': 'तारीख',
      'start': 'प्रारंभ',
      'end': 'समाप्त',
      'status': 'स्थिति',
      'description': 'विवरण',
      'name': 'नाम',
      'phone': 'फ़ोन',
      'email': 'ईमेल',
      'address': 'पता',
      'actions': 'कार्रवाई',
      'total': 'कुल',
      'balance': 'शेष',
      'amount': 'राशि',
      'all': 'सभी',
      'available_schemes': 'उपलब्ध योजनाएं',
      'no_schemes_available': 'कोई योजना उपलब्ध नहीं है',
      'add_scheme': 'योजना जोड़ें',
      'scheme_name': 'योजना का नाम',
      'scheme_type': 'योजना का प्रकार',
      'paid_visits': 'भुगतान की गई विज़िट',
      'free_visits': 'मुफ़्त विज़िट',
      'discount_percentage': 'छूट प्रतिशत',
      'voucher_no': 'वाउचर संख्या',
      'add_voucher': 'वाउचर जोड़ें',
      'services': 'सेवाएं',
      'customer_types': 'ग्राहक प्रकार',
      'vehicle_types': 'वाहन प्रकार',
      'save_scheme': 'योजना सहेजें',
      'scheme_created': 'योजना बनाई गई',
      'enter_scheme_details_error': 'योजना का नाम और प्रकार दर्ज करें',
      'booking_details': 'बुकिंग विवरण',
      'customer_name': 'ग्राहक का नाम',
      'phone_number': 'फ़ोन नंबर',
      'vehicle_number': 'वाहन संख्या',
      'select_service': 'सेवा चुनें',
      'select_date': 'तारीख चुनें',
      'time_slot': 'समय स्लॉट',
      'booking_created': 'बुकिंग बनाई गई',
      'booking_failed': 'बुकिंग विफल रही',
      'bookings_list': 'बुकिंग सूची',
      'no_bookings_found': 'कोई बुकिंग नहीं मिली',
      'receipts': 'रसीदें',
      'no_receipts_found': 'कोई रसीद नहीं मिली',
      'collection': 'संग्रह',
      'collect_payment': 'भुगतान एकत्र करें',
      'payment_collected': 'भुगतान एकत्र किया गया',
      'add_complaint': 'शिकायत जोड़ें',
      'subject': 'विषय',
      'complaint_created': 'शिकायत दर्ज की गई',
      'no_complaints_found': 'कोई शिकायत नहीं मिली',
      'add_new_customer': 'नया ग्राहक जोड़ें',
      'edit_customer': 'ग्राहक संपादित करें',
      'customer_created': 'ग्राहक बनाया गया',
      'customer_updated': 'ग्राहक अपडेट किया गया',
      'no_customers_found': 'कोई ग्राहक नहीं मिला',
      'create_invoice': 'इनवॉइस बनाएं',
      'invoice_details': 'इनवॉइस विवरण',
      'invoice_created': 'इनवॉइस बनाया गया',
      'invoice_number': 'इनवॉइस संख्या',
      'subtotal': 'उप-योग',
      'tax': 'कर',
      'grand_total': 'कुल योग',
      'select_vehicle': 'वाहन चुनें',
      'select_scheme': 'योजना चुनें',
      'job_details': 'कार्य विवरण',
      'revenue_report': 'राजस्व रिपोर्ट',
      'jobs_report': 'कार्य रिपोर्ट',
      'sales_summary': 'बिक्री सारांश',
      'search_by_vehicle_number': 'वाहन संख्या द्वारा खोजें',
      'vehicle_details': 'वाहन विवरण',
      'msg_welcome': 'नमस्ते {customer}! 👋 हमारे सर्विस सेंटर में आपका स्वागत है। आपका वाहन {vehicle} सुरक्षित रूप से पहुंच गया है और विशेषज्ञ हाथों में है। हम आपको अपडेट करते रहेंगे!',
      'msg_ready_alert': 'नमस्ते {customer}! 🎉 खुशखबरी! आपका वाहन {vehicle} लेने के लिए तैयार है। कृपया जल्द से जल्द इसे ले जाएं।',
      'msg_thanks': 'प्रिय {customer}, 🙏 अपने वाहन {vehicle} को हम पर भरोसा करने के लिए धन्यवाद। हम आशा करते हैं कि आपका अनुभव अच्छा रहा। फिर मिलेंगे!',
    },
    'ur': {
      'dashboard': 'ڈیش بورڈ',
      'menu': 'مینو',
      'new_job': 'نیا کام',
      'book_now': 'ابھی بک کریں',
      'bookings': 'بکنگ',
      'bill': 'بل',
      'outstanding': 'بقایا جات',
      'receipt': 'رسید',
      'customers': 'گاہکوں',
      'schemes': 'اسکیمیں',
      'reports': 'رپورٹیں',
      'vehicle': 'گاڑی',
      'notifications': 'نوٹیفیکیشنز',
      'language': 'زبان',
      'complaints': 'شکایات',
      'logout': 'لاگ آؤٹ',
      'welcome_back': 'خوش آمدید',
      'add_extras': 'اضافی چیزیں شامل کریں',
      'services_subtotal': 'سروسز کا سب ٹوٹل',
      'scheme_discount': 'اسکیم ڈسکاؤنٹ',
      'manual_discount': 'دستی ڈسکاؤنٹ',
      'bill_summary': 'بل کا خلاصہ',
      'amount_collected': 'جمع شدہ رقم',
      'taxes': 'ٹیکس',
      'quick_menu': 'فوری مینو',
      'login': 'لاگ ان',
      'username_or_email': 'صارف نام یا ای میل',
      'enter_username_or_email': 'صارف نام یا ای میل درج کریں',
      'password': 'پاس ورڈ',
      'enter_password': 'پاس ورڈ درج کریں',
      'remember_me': 'مجھے یاد رکھیں',
      'forgot_password': 'پاس ورڈ بھول گئے؟',
      'sign_in': 'سائن ان کریں',
      'logout_confirm': 'کیا آپ واقعی لاگ آؤٹ کرنا چاہتے ہیں؟',
      'cancel': 'منسوخ کریں',
      'branch_admin': 'برانچ ایڈمن',
      'company_admin': 'کمپنی ایڈمن',
      'recent_invoices': 'حالیہ انوائسز',
      'select_language': 'زبان منتخب کریں',
      'admin_login': 'ایڈمن لاگ ان',
      'username': 'صارف نام',
      'please_enter_credentials': 'براہ کرم صارف نام اور پاس ورڈ درج کریں',
      'todays_summary': 'آج کا خلاصہ',
      'overview': 'جائزہ',
      'todays_recent_jobs': 'آج کے حالیہ کام',
      'revenue': 'آمدنی',
      'collected': 'جمع شدہ',
      'jobs': 'کام',
      'due': 'واجب الادا',
      'paid': 'ادا شدہ',
      'invoices_count': 'انوائسز',
      'exit_app': 'ایپ سے باہر نکلیں',
      'exit_confirm': 'کیا آپ ایپ بند کرنا چاہتے ہیں؟',
      'exit': 'باہر نکلیں',
      'bills': 'بلز',
      'vehicle_search': 'گاڑی کی تلاش',
      'not_authenticated': 'تصدیق شدہ نہیں ہے',
      'network_error': 'نیٹ ورک کی خرابی',
      'failed_to_load': 'لوڈ کرنے میں ناکام',
      'retry': 'دوبارہ کوشش کریں',
      'save': 'محفوظ کریں',
      'saving': 'محفوظ ہو رہا ہے...',
      'delete': 'حذف کریں',
      'add': 'شامل کریں',
      'create': 'تخلیق کریں',
      'date': 'تاریخ',
      'start': 'شروع',
      'end': 'ختم',
      'status': 'حیثیت',
      'description': 'تفصیل',
      'name': 'نام',
      'phone': 'فون',
      'email': 'ای میل',
      'address': 'پتہ',
      'actions': 'اقدامات',
      'total': 'کل',
      'balance': 'باقی رقم',
      'amount': 'رقم',
      'all': 'تمام',
      'available_schemes': 'دستیاب اسکیمیں',
      'no_schemes_available': 'کوئی اسکیم دستیاب نہیں ہے',
      'add_scheme': 'اسکیم شامل کریں',
      'scheme_name': 'اسکیم کا نام',
      'scheme_type': 'اسکیم کی قسم',
      'paid_visits': 'ادا شدہ وزٹ',
      'free_visits': 'مفت وزٹ',
      'discount_percentage': 'چھوٹ کا فیصد',
      'voucher_no': 'واؤچر نمبر',
      'add_voucher': 'واؤچر شامل کریں',
      'services': 'خدمات',
      'customer_types': 'گاہکوں کی اقسام',
      'vehicle_types': 'گاڑیوں کی اقسام',
      'save_scheme': 'اسکیم محفوظ کریں',
      'scheme_created': 'اسکیم بنا دی گئی',
      'enter_scheme_details_error': 'اسکیم کا نام اور قسم درج کریں',
      'booking_details': 'بکنگ کی تفصیلات',
      'customer_name': 'گاہک کا نام',
      'phone_number': 'فون نمبر',
      'vehicle_number': 'گاڑی کا نمبر',
      'select_service': 'سروس منتخب کریں',
      'select_date': 'تاریخ منتخب کریں',
      'time_slot': 'وقت کا سلاٹ',
      'booking_created': 'بکنگ ہو گئی',
      'booking_failed': 'بکنگ ناکام ہو گئی',
      'bookings_list': 'بکنگ کی فہرست',
      'no_bookings_found': 'کوئی booking نہیں ملی',
      'receipts': 'رسیدیں',
      'no_receipts_found': 'کوئی رسید نہیں ملی',
      'collection': 'مجموعہ',
      'collect_payment': 'ادائیگی جمع کریں',
      'payment_collected': 'ادائیگی جمع کرلی گئی',
      'add_complaint': 'شکایت درج کریں',
      'subject': 'موضوع',
      'complaint_created': 'شکایت درج کرلی گئی',
      'no_complaints_found': 'کوئی شکایت نہیں ملی',
      'add_new_customer': 'نیا گاہک شامل کریں',
      'edit_customer': 'گاہک کی معلومات تبدیل کریں',
      'customer_created': 'گاہک کا پروفائل بن گیا',
      'customer_updated': 'گاہک کی معلومات اپ ڈیٹ ہوگئیں',
      'no_customers_found': 'کوئی گاہک نہیں ملا',
      'create_invoice': 'انوائس بنائیں',
      'invoice_details': 'انوائس کی تفصیلات',
      'invoice_created': 'انوائس بنا دی گئی',
      'invoice_number': 'انوائس نمبر',
      'subtotal': 'ذیلی کل',
      'tax': 'ٹیکس',
      'grand_total': 'کل مجموعہ',
      'select_vehicle': 'گاڑی منتخب کریں',
      'select_scheme': 'اسکیم منتخب کریں',
      'job_details': 'کام کی تفصیلات',
      'revenue_report': 'آمدنی کی رپورٹ',
      'jobs_report': 'کاموں کی رپورٹ',
      'sales_summary': 'فروخت का خلاصہ',
      'search_by_vehicle_number': 'گاڑی کے نمبر سے تلاش کریں',
      'vehicle_details': 'گاڑی کی تفصیلات',
      'msg_welcome': 'ہیلو {customer}! 👋 ہمارے سروس سینٹر میں خوش آمدید۔ آپ کی گاڑی {vehicle} محفوظ طریقے سے پہنچ گئی اور ماہر ہاتھوں میں ہے۔ ہم آپ کو اپڈیٹ کرتے رہیں گے!',
      'msg_ready_alert': 'ہیلو {customer}! 🎉 خوشخبری! آپ کی گاڑی {vehicle} وصول کرنے کے لیے تیار ہے۔ براہ کرم جلد از جلد لے جائیں۔',
      'msg_thanks': 'عزیز {customer}، 🙏 اپنی گاڑی {vehicle} کے لیے ہم پر بھروسہ کرنے کا شکریہ۔ امید ہے کہ آپ کا تجربہ شاندار رہا۔ پھر ملیں گے!',
    },
    'ar': {
      'dashboard': 'لوحة القيادة',
      'menu': 'القائمة',
      'new_job': 'عمل جديد',
      'book_now': 'احجز الآن',
      'bookings': 'الحجوزات',
      'bill': 'الفاتورة',
      'outstanding': 'المستحقات',
      'receipt': 'الإيصال',
      'customers': 'العملاء',
      'schemes': 'العروض',
      'reports': 'التقارير',
      'vehicle': 'المركبة',
      'notifications': 'الإشعارات',
      'language': 'اللغة',
      'complaints': 'الشكاوى',
      'logout': 'تسجيل الخروج',
      'welcome_back': 'مرحباً بعودتك',
      'add_extras': 'إضافة إضافات',
      'services_subtotal': 'المجموع الفرعي للخدمات',
      'scheme_discount': 'خصم العرض',
      'manual_discount': 'خصم يدوي',
      'bill_summary': 'ملخص الفاتورة',
      'amount_collected': 'المبلغ المحصل',
      'taxes': 'الضرائب',
      'quick_menu': 'القائمة السريعة',
      'login': 'تسجيل الدخول',
      'username_or_email': 'اسم المستخدم أو البريد الإلكتروني',
      'enter_username_or_email': 'أدخل اسم المستخدم أو البريد الإلكتروني',
      'password': 'كلمة المرور',
      'enter_password': 'أدخل كلمة المرور',
      'remember_me': 'تذكرني',
      'forgot_password': 'هل نسيت كلمة المرور؟',
      'sign_in': 'تسجيل الدخول',
      'logout_confirm': 'هل أنت متأكد أنك تريد تسجيل الخروج؟',
      'cancel': 'إلغاء',
      'branch_admin': 'مسؤول الفرع',
      'company_admin': 'مسؤول الشركة',
      'recent_invoices': 'الفواتير الأخيرة',
      'select_language': 'اختر اللغة',
      'admin_login': 'دخول المسؤول',
      'username': 'اسم المستخدم',
      'please_enter_credentials': 'يرجى إدخال اسم المستخدم وكلمة المرور',
      'todays_summary': 'ملخص اليوم',
      'overview': 'نظرة عامة',
      'todays_recent_jobs': 'أعمال اليوم الأخيرة',
      'revenue': 'الإيرادات',
      'collected': 'المحصلة',
      'jobs': 'الأعمال',
      'due': 'المستحقة',
      'paid': 'المدفوعة',
      'invoices_count': 'الفواتير',
      'exit_app': 'الخروج من التطبيق',
      'exit_confirm': 'هل تريد إغلاق التطبيق؟',
      'exit': 'خروج',
      'bills': 'الفواتير',
      'vehicle_search': 'البحث عن المركبة',
      'not_authenticated': 'غير مصرح',
      'network_error': 'خطأ في الشبكة',
      'failed_to_load': 'فشل التحميل',
      'retry': 'إعادة المحاولة',
      'save': 'حفظ',
      'saving': 'جاري الحفظ...',
      'delete': 'حذف',
      'add': 'إضافة',
      'create': 'إنشاء',
      'date': 'التاريخ',
      'start': 'البدء',
      'end': 'الانتهاء',
      'status': 'الحالة',
      'description': 'الوصف',
      'name': 'الاسم',
      'phone': 'الهاتف',
      'email': 'البريد الإلكتروني',
      'address': 'العنوان',
      'actions': 'الإجراءات',
      'total': 'الإجمالي',
      'balance': 'الرصيد',
      'amount': 'المبلغ',
      'all': 'الكل',
      'available_schemes': 'العروض المتاحة',
      'no_schemes_available': 'لا توجد عروض متاحة',
      'add_scheme': 'إضافة عرض',
      'scheme_name': 'اسم العرض',
      'scheme_type': 'نوع العرض',
      'paid_visits': 'الزيارات المدفوعة',
      'free_visits': 'الزيارات المجانية',
      'discount_percentage': 'نسبة الخصم',
      'voucher_no': 'رقم القسيمة',
      'add_voucher': 'إضافة قسيمة',
      'services': 'الخدمات',
      'customer_types': 'أنواع العملاء',
      'vehicle_types': 'أنواع المركبات',
      'save_scheme': 'حفظ العرض',
      'scheme_created': 'تم إنشاء العرض',
      'enter_scheme_details_error': 'أدخل اسم ونوع العرض',
      'booking_details': 'تفاصيل الحجز',
      'customer_name': 'اسم العميل',
      'phone_number': 'رقم الهاتف',
      'vehicle_number': 'رقم المركبة',
      'select_service': 'اختر الخدمة',
      'select_date': 'اختر التاريخ',
      'time_slot': 'الفترة الزمنية',
      'booking_created': 'تم إنشاء الحجز',
      'booking_failed': 'فشل الحجز',
      'bookings_list': 'قائمة الحجوزات',
      'no_bookings_found': 'لم يتم العثور على حجوزات',
      'receipts': 'الإيصالات',
      'no_receipts_found': 'لم يتم العثور على إيصالات',
      'collection': 'التحصيل',
      'collect_payment': 'تحصيل الدفعة',
      'payment_collected': 'تم تحصيل الدفعة',
      'add_complaint': 'إضافة شكوى',
      'subject': 'الموضوع',
      'complaint_created': 'تم تسجيل الشكوى',
      'no_complaints_found': 'لم يتم العثور على شكاوى',
      'add_new_customer': 'إضافة عميل جديد',
      'edit_customer': 'تعديل العميل',
      'customer_created': 'تم إنشاء العميل',
      'customer_updated': 'تم تحديث العميل',
      'no_customers_found': 'لم يتم العثور على عملاء',
      'create_invoice': 'إنشاء فاتورة',
      'invoice_details': 'تفاصيل الفاتورة',
      'invoice_created': 'تم إنشاء الفاتورة',
      'invoice_number': 'رقم الفاتورة',
      'subtotal': 'المجموع الفرعي',
      'tax': 'الضريبة',
      'grand_total': 'المجموع الكلي',
      'select_vehicle': 'اختر المركبة',
      'select_scheme': 'اختر العرض',
      'job_details': 'تفاصيل العمل',
      'revenue_report': 'تقرير الإيرادات',
      'jobs_report': 'تقرير الأعمال',
      'sales_summary': 'ملخص المبيعات',
      'search_by_vehicle_number': 'البحث برقم المركبة',
      'vehicle_details': 'تفاصيل المركبة',
      'msg_welcome': 'مرحباً {customer}! 👋 أهلاً بك في مركز الخدمة. وصلت سيارتك {vehicle} بأمان وهي في أيدٍ أمينة. سنُبقيك على اطلاع!',
      'msg_ready_alert': 'مرحباً {customer}! 🎉 خبر رائع! سيارتك {vehicle} جاهزة للاستلام. يرجى التفضل باستلامها في أقرب وقت.',
      'msg_thanks': 'عزيزي {customer}، 🙏 شكراً لثقتك بنا في خدمة سيارتك {vehicle}. نأمل أن تكون التجربة رائعة. نتطلع إلى خدمتك مجدداً!',
    },
    'bn': {
      'dashboard': 'ড্যাশবোর্ড',
      'menu': 'মেনু',
      'new_job': 'নতুন কাজ',
      'book_now': 'এখনই বুক করুন',
      'bookings': 'বুকিং সমূহ',
      'bill': 'বিল',
      'outstanding': 'বকেয়া',
      'receipt': 'রশিদ',
      'customers': 'গ্রাহক সমূহ',
      'schemes': 'স্কিম সমূহ',
      'reports': 'রিপোর্ট সমূহ',
      'vehicle': 'যানবাহন',
      'notifications': 'বিজ্ঞপ্তি',
      'language': 'ভাষা',
      'complaints': 'অভিযোগ সমূহ',
      'logout': 'লগ আউট',
      'welcome_back': 'স্বাগতম',
      'add_extras': 'অতিরিক্ত যোগ করুন',
      'services_subtotal': 'সেবাসমূহের উপ-মোট',
      'scheme_discount': 'স্কিম ডিসকাউন্ট',
      'manual_discount': 'ম্যানুয়াল ডিসকাউন্ট',
      'bill_summary': 'বিল সংক্ষিপ্তসার',
      'amount_collected': 'সংগৃহীত টাকা',
      'taxes': 'কর',
      'quick_menu': 'কুইক মেনু',
      'login': 'লগইন',
      'username_or_email': 'ইউজারনেম বা ইমেল',
      'enter_username_or_email': 'ইউজারনেম বা ইমেল লিখুন',
      'password': 'পাসওয়ার্ড',
      'enter_password': 'পাসওয়ার্ড লিখুন',
      'remember_me': 'আমাকে মনে রাখুন',
      'forgot_password': 'পাসওয়ার্ড ভুলে গেছেন?',
      'sign_in': 'সাইন ইন',
      'logout_confirm': 'আপনি কি নিশ্চিত যে লগ আউট করতে চান?',
      'cancel': 'বাতিল',
      'branch_admin': 'শাখা প্রশাসক',
      'company_admin': 'কোম্পানি প্রশাসক',
      'recent_invoices': 'সাম্প্রতিক চালান',
      'select_language': 'ভাষা নির্বাচন করুন',
      'admin_login': 'অ্যাডমিন লগইন',
      'username': 'ইউজারনেম',
      'please_enter_credentials': 'অনুগ্রহ করে ইউজারনেম এবং পাসওয়ার্ড লিখুন',
      'todays_summary': 'আজকের সংক্ষিপ্ত বিবরণ',
      'overview': 'অভারভিউ',
      'todays_recent_jobs': 'আজকের সাম্প্রতিক কাজ',
      'revenue': 'রাজস্ব',
      'collected': 'সংগৃহীত',
      'jobs': 'কাজ সমূহ',
      'due': 'বকেয়া',
      'paid': 'পরিশোধিত',
      'invoices_count': 'চালান',
      'exit_app': 'অ্যাপ থেকে প্রস্থান করুন',
      'exit_confirm': 'আপনি কি অ্যাপটি বন্ধ করতে চান?',
      'exit': 'প্রস্থান',
      'bills': 'বিল সমূহ',
      'vehicle_search': 'যানবাহন অনুসন্ধান',
      'not_authenticated': 'অনুমোদিত নয়',
      'network_error': 'নেটওয়ার্ক ত্রুটি',
      'failed_to_load': 'লোড হতে ব্যর্থ হয়েছে',
      'retry': 'পুনরায় চেষ্টা করুন',
      'save': 'সংরক্ষণ করুন',
      'saving': 'সংরক্ষণ করা হচ্ছে...',
      'delete': 'মুছে ফেলুন',
      'add': 'যোগ করুন',
      'create': 'তৈরি করুন',
      'date': 'তারিখ',
      'start': 'শুরু',
      'end': 'শেষ',
      'status': 'অবস্থা',
      'description': 'বর্ণنا',
      'name': 'নাম',
      'phone': 'ফোন',
      'email': 'ইমেল',
      'address': 'ঠিকানা',
      'actions': 'পদক্ষেপ সমূহ',
      'total': 'মোট',
      'balance': 'অবশিষ্ট',
      'amount': 'পরিমাণ',
      'all': 'সব',
      'available_schemes': 'উপলব্ধ স্কিম সমূহ',
      'no_schemes_available': 'কোন স্কিম উপলব্ধ নেই',
      'add_scheme': 'স্কিম যোগ করুন',
      'scheme_name': 'স্কিমের নাম',
      'scheme_type': 'স্কিমের ধরন',
      'paid_visits': 'পেইড ভিজিট',
      'free_visits': 'ফ্রি ভিজিট',
      'discount_percentage': 'ছাড়ের শতাংশ',
      'voucher_no': 'ভাউচার নম্বর',
      'add_voucher': 'ভাউচার যোগ করুন',
      'services': 'সেবা সমূহ',
      'customer_types': 'গ্রাহকের ধরন',
      'vehicle_types': 'যানবাহনের ধরন',
      'save_scheme': 'স্কিম সংরক্ষণ করুন',
      'scheme_created': 'স্কিম তৈরি হয়েছে',
      'enter_scheme_details_error': 'স্কিমের নাম এবং ধরন লিখুন',
      'booking_details': 'বুকিংয়ের বিবরণ',
      'customer_name': 'গ্রাহকের নাম',
      'phone_number': 'ফোন নম্বর',
      'vehicle_number': 'যানবাহন নম্বর',
      'select_service': 'সেবা নির্বাচন করুন',
      'select_date': 'তারিখ নির্বাচন করুন',
      'time_slot': 'समय স্লট',
      'booking_created': 'বুকিং তৈরি হয়েছে',
      'booking_failed': 'বুকিং ব্যর্থ হয়েছে',
      'bookings_list': 'বুকিং তালিকা',
      'no_bookings_found': 'কোন বুকিং পাওয়া যায়নি',
      'receipts': 'রশিদ সমূহ',
      'no_receipts_found': 'কোন রশিদ পাওয়া যায়নি',
      'collection': 'সংগ্রহ',
      'collect_payment': 'পেমেন্ট সংগ্রহ করুন',
      'payment_collected': 'পেমেন্ট সংগৃহীত হয়েছে',
      'add_complaint': 'অভিযোগ যোগ করুন',
      'subject': 'বিষয়',
      'complaint_created': 'অভিযোগ নিবন্ধিত হয়েছে',
      'no_complaints_found': 'কোন অভিযোগ পাওয়া যায়নি',
      'add_new_customer': 'নতুন গ্রাহক যোগ করুন',
      'edit_customer': 'গ্রাহক সম্পাদনা করুন',
      'customer_created': 'গ্রাহক তৈরি হয়েছে',
      'customer_updated': 'গ্রাহক আপডেট হয়েছে',
      'no_customers_found': 'কোন গ্রাহক পাওয়া যায়নি',
      'create_invoice': 'চালান তৈরি করুন',
      'invoice_details': 'চালানের বিবরণ',
      'invoice_created': 'চালান তৈরি হয়েছে',
      'invoice_number': 'চালান নম্বর',
      'subtotal': 'সাবটোটাল',
      'tax': 'ট্যাক্স',
      'grand_total': 'সর্বমোট',
      'select_vehicle': 'যানবাহন নির্বাচন করুন',
      'select_scheme': 'স্কিম নির্বাচন করুন',
      'job_details': 'কাজের বিবরণ',
      'revenue_report': 'রাজস্ব রিপোর্ট',
      'jobs_report': 'কাজের রিপোর্ট',
      'sales_summary': 'বিক্রয় সারসংক্ষেপ',
      'search_by_vehicle_number': 'যানবাহন নম্বর দিয়ে অনুসন্ধান করুন',
      'vehicle_details': 'যানবাহনের বিবরণ',
      'msg_welcome': 'হ্যালো {customer}! 👋 আমাদের সার্ভিস সেন্টারে আপনাকে স্বাগতম। আপনার গাড়ি {vehicle} নিরাপদে এসেছে এবং বিশেষজ্ঞ হাতে আছে। আমরা আপনাকে আপডেট করতে থাকব!',
      'msg_ready_alert': 'হ্যালো {customer}! 🎉 সুখবর! আপনার গাড়ি {vehicle} পিকআপের জন্য প্রস্তুত। অনুগ্রহ করে যত তাড়াতাড়ি সম্ভব সংগ্রহ করুন।',
      'msg_thanks': 'প্রিয় {customer}, 🙏 আপনার গাড়ি {vehicle} আমাদের উপর বিশ্বাস রাখার জন্য ধন্যবাদ। আশা করি দারুণ অভিজ্ঞতা হয়েছে। আবার দেখা হবে!',
    },
    'ml': {
      'dashboard': 'ഡാഷ്‌ബോർഡ്',
      'menu': 'മെനു',
      'new_job': 'പുതിയ ജോലി',
      'book_now': 'ഇപ്പോൾ ബുക്ക് ചെയ്യുക',
      'bookings': 'ബുക്കിംഗുകൾ',
      'bill': 'ബിൽ',
      'outstanding': 'കുടിശ്ശിക',
      'receipt': 'രസീത്',
      'customers': 'ഉപഭോക്താക്കൾ',
      'schemes': 'സ്കീമുകൾ',
      'reports': 'റിപ്പോർട്ടുകൾ',
      'vehicle': 'വാഹനം',
      'notifications': 'അറിയിപ്പുകൾ',
      'language': 'ഭാഷ',
      'complaints': 'പരാതികൾ',
      'logout': 'ലോഗ് ഔട്ട്',
      'welcome_back': 'സ്വാഗതം',
      'add_extras': 'അധികമായി ചേർക്കുക',
      'services_subtotal': 'സേവനങ്ങളുടെ സബ്‌ടോട്ടൽ',
      'scheme_discount': 'സ്കീം ഡിസ്കൗണ്ട്',
      'manual_discount': 'മാനുവൽ ഡിസ്കൗണ്ട്',
      'bill_summary': 'ബില്ല് വിവരണം',
      'amount_collected': 'ശേഖരിച്ച തുക',
      'taxes': 'നികുതികൾ',
      'quick_menu': 'ക്വിക്ക് മെനു',
      'login': 'ലോഗിൻ',
      'username_or_email': 'ഉപയോക്തൃനാമം അല്ലെങ്കിൽ ഇമെയിൽ',
      'enter_username_or_email': 'ഉപയോക്തൃനാമം അല്ലെങ്കിൽ ഇമെയിൽ നൽകുക',
      'password': 'പാസ്‌വേഡ്',
      'enter_password': 'പാസ്‌വേഡ് നൽകുക',
      'remember_me': 'എന്നെ ഓർമ്മിക്കുക',
      'forgot_password': 'പാസ്‌വേഡ് മറന്നുപോയോ?',
      'sign_in': 'സൈൻ ഇൻ',
      'logout_confirm': 'നിങ്ങൾ ലോഗ് ഔട്ട് ചെയ്യാൻ ആഗ്രഹിക്കുന്നുവെന്ന് ഉറപ്പാണോ?',
      'cancel': 'റദ്ദാക്കുക',
      'branch_admin': 'ബ്രാഞ്ച് അഡ്മിൻ',
      'company_admin': 'കമ്പനി അഡ്മിൻ',
      'recent_invoices': 'അടുത്തകാലത്തെ ബില്ലുകൾ',
      'select_language': 'ഭാഷ തിരഞ്ഞെടുക്കുക',
      'admin_login': 'അഡ്മിൻ ലോഗിൻ',
      'username': 'ഉപയോക്തൃനാമം',
      'please_enter_credentials': 'ദയവായി ഉപയോക്തൃനാമവും പാസ്‌വേഡും നൽകുക',
      'todays_summary': 'ഇന്നത്തെ ചുരുക്കം',
      'overview': 'അവലോകനം',
      'todays_recent_jobs': 'ഇന്നത്തെ സമീപകാല ജോലികൾ',
      'revenue': 'വരുമാനം',
      'collected': 'ശേഖരിച്ചത്',
      'jobs': 'ജോലികൾ',
      'due': 'ബാക്കി',
      'paid': 'അടച്ചു',
      'invoices_count': 'ഇൻവോയ്സുകൾ',
      'exit_app': 'ആപ്പിൽ നിന്ന് പുറത്തുകടക്കുക',
      'exit_confirm': 'നിങ്ങൾക്ക് ആപ്പ് അടക്കണോ?',
      'exit': 'പുറത്തുകടക്കുക',
      'bills': 'ബില്ലുകൾ',
      'vehicle_search': 'വാഹനം തിരയുക',
      'not_authenticated': 'അംഗീകരിക്കപ്പെട്ടിട്ടില്ല',
      'network_error': 'നെറ്റ്‌വർക്ക് തകരാർ',
      'failed_to_load': 'ലോഡ് ചെയ്യാൻ കഴിഞ്ഞില്ല',
      'retry': 'വീണ്ടും ശ്രമിക്കുക',
      'save': 'സൂക്ഷിക്കുക',
      'saving': 'സൂക്ഷിക്കുന്നു...',
      'delete': 'ഇല്ലാതാക്കുക',
      'add': 'ചേർക്കുക',
      'create': 'നിർമ്മിക്കുക',
      'date': 'തീയതി',
      'start': 'ആരംഭം',
      'end': 'അവസാനം',
      'status': 'നില',
      'description': 'വിവരണം',
      'name': 'പേര്',
      'phone': 'ഫോൺ',
      'email': 'ഇമെയിൽ',
      'address': 'മേൽവിലാസം',
      'actions': 'നടപടികൾ',
      'total': 'ആകെ',
      'balance': 'ബാക്കി',
      'amount': 'തുക',
      'all': 'എല്ലാം',
      'available_schemes': 'ലഭ്യമായ സ്കീമുകൾ',
      'no_schemes_available': 'സ്കീമുകൾ ഒന്നും ലഭ്യമായിട്ടില്ല',
      'add_scheme': 'സ്കീം ചേർക്കുക',
      'scheme_name': 'സ്കീമിന്റെ പേര്',
      'scheme_type': 'സ്കീമിന്റെ തരം',
      'paid_visits': 'പെയ്ഡ് സന്ദർശനങ്ങൾ',
      'free_visits': 'സൗജന്യ സന്ദർശനങ്ങൾ',
      'discount_percentage': 'ഡിസ്കൗണ്ട് ശതമാനം',
      'voucher_no': 'വൗച്ചർ നമ്പർ',
      'add_voucher': 'വൗച്ചർ ചേർക്കുക',
      'services': 'സേവനങ്ങൾ',
      'customer_types': 'ഉപഭോക്തൃ തരങ്ങൾ',
      'vehicle_types': 'വാഹന തരങ്ങൾ',
      'save_scheme': 'സ്കീം സൂക്ഷിക്കുക',
      'scheme_created': 'സ്കീം നിർമ്മിച്ചു',
      'enter_scheme_details_error': 'സ്കീമിന്റെ പേരും തരവും നൽകുക',
      'booking_details': 'ബുക്കിംഗ് വിവരങ്ങൾ',
      'customer_name': 'ഉപഭോക്താവിന്റെ പേര്',
      'phone_number': 'ഫോൺ നമ്പർ',
      'vehicle_number': 'വാഹന നമ്പർ',
      'select_service': 'സേവനം തിരഞ്ഞെടുക്കുക',
      'select_date': 'തീയതി തിരഞ്ഞെടുക്കുക',
      'time_slot': 'സമയ ക്രമം',
      'booking_created': 'ബുക്കിംഗ് വിജയകരമായി',
      'booking_failed': 'ബുക്കിംഗ് പരാജയപ്പെട്ടു',
      'bookings_list': 'ബുക്കിംഗ് ലിസ്റ്റ്',
      'no_bookings_found': 'ബുക്കിംഗുകൾ ഒന്നും കണ്ടെത്തിയില്ല',
      'receipts': 'രസീതുകൾ',
      'no_receipts_found': 'രസീതുകൾ ഒന്നും കണ്ടെത്തിയില്ല',
      'collection': 'ശേഖരണം',
      'collect_payment': 'പണം സ്വീകരിക്കുക',
      'payment_collected': 'പണം സ്വീകരിച്ചു',
      'add_complaint': 'പരാതി നൽകുക',
      'subject': 'വിഷയം',
      'complaint_created': 'പരാതി രജിസ്റ്റർ ചെയ്തു',
      'no_complaints_found': 'പരാതികൾ ഒന്നും കണ്ടെത്തിയില്ല',
      'add_new_customer': 'പുതിയ ഉപഭോക്താവിനെ ചേർക്കുക',
      'edit_customer': 'ഉപഭോക്താവിനെ തിരുത്തുക',
      'customer_created': 'ഉപഭോക്താവിനെ ചേർത്തു',
      'customer_updated': 'ഉപഭോക്താവിനെ പുതുക്കി',
      'no_customers_found': 'ഉപഭോക്താക്കളെ കണ്ടെത്തിയില്ല',
      'create_invoice': 'ബില്ല് നിർമ്മിക്കുക',
      'invoice_details': 'ബില്ല് വിവരങ്ങൾ',
      'invoice_created': 'ബില്ല് നിർമ്മിച്ചു',
      'invoice_number': 'ബില്ല് നമ്പർ',
      'subtotal': 'സബ്‌ടോട്ടൽ',
      'tax': 'നികുതി',
      'grand_total': 'ആകെ തുക',
      'select_vehicle': 'വാഹനം തിരഞ്ഞെടുക്കുക',
      'select_scheme': 'സ്കീം തിരഞ്ഞെടുക്കുക',
      'job_details': 'ജോലി വിവരങ്ങൾ',
      'revenue_report': 'വരുമാന റിപ്പോർട്ട്',
      'jobs_report': 'ജോലി റിപ്പോർട്ട്',
      'sales_summary': 'വില്പന വിവരണം',
      'search_by_vehicle_number': 'വാഹന നമ്പർ ഉപയോഗിച്ച് തിരയുക',
      'vehicle_details': 'വാഹന വിവരങ്ങൾ',
      'msg_welcome': 'ഹലോ {customer}! 👋 ഞങ്ങളുടെ സർവ്വീസ് സെന്ററിലേക്ക് സ്വാഗതം. താങ്കളുടെ വാഹനം {vehicle} സുരക്ഷിതമായി എത്തിയിരിക്കുന്നു. ഞങ്ങൾ ഉടൻ അറിയിക്കാം!',
      'msg_ready_alert': 'ഹലോ {customer}! 🎉 സന്തോഷ വർത്തമാനം! താങ്കളുടെ വാഹനം {vehicle} പിക്കപ്പ് ചെയ്യാൻ തയ്യാർ. ദയവായി യഥാവസരം ശേഖരിക്കുക.',
      'msg_thanks': 'പ്രിയ {customer}, 🙏 {vehicle} വാഹനം ഞങ്ങളെ ഏൽപ്പിച്ചതിന് നന്ദി. മികച്ച അനുഭവം ഉണ്ടായെന്ന് ആശിക്കുന്നു. വീണ്ടും കാണാം!',
    },
    'ru': {
      'dashboard': 'Панель управления',
      'menu': 'Меню',
      'new_job': 'Новый заказ',
      'book_now': 'Забронировать',
      'bookings': 'Бронирования',
      'bill': 'Счет',
      'outstanding': 'Задолженность',
      'receipt': 'Квитанция',
      'customers': 'Клиенты',
      'schemes': 'Схемы',
      'reports': 'Отчеты',
      'vehicle': 'Транспорт',
      'notifications': 'Уведомления',
      'language': 'Язык',
      'complaints': 'Жалобы',
      'logout': 'Выйти',
      'welcome_back': 'С возвращением',
      'add_extras': 'Добавить доп. услуги',
      'services_subtotal': 'Подытог услуг',
      'scheme_discount': 'Скидка по схеме',
      'manual_discount': 'Ручная скидка',
      'bill_summary': 'Итого по счету',
      'amount_collected': 'Собрано средств',
      'taxes': 'Налоги',
      'quick_menu': 'Быстрое меню',
      'login': 'Войти',
      'username_or_email': 'Имя пользователя или Email',
      'enter_username_or_email': 'Введите имя пользователя или Email',
      'password': 'Пароль',
      'enter_password': 'Введите пароль',
      'remember_me': 'Запомнить меня',
      'forgot_password': 'Забыли пароль?',
      'sign_in': 'Войти',
      'logout_confirm': 'Вы уверены, что хотите выйти?',
      'cancel': 'Отмена',
      'branch_admin': 'Админ филиала',
      'company_admin': 'Админ компании',
      'recent_invoices': 'Последние счета',
      'select_language': 'Выберите язык',
      'admin_login': 'Вход для админа',
      'username': 'Имя пользователя',
      'please_enter_credentials': 'Пожалуйста, введите логин и пароль',
      'todays_summary': 'Итоги за сегодня',
      'overview': 'Обзор',
      'todays_recent_jobs': 'Недавние заказы за сегодня',
      'revenue': 'Выручка',
      'collected': 'Собрано',
      'jobs': 'Заказы',
      'due': 'К оплате',
      'paid': 'Оплачено',
      'invoices_count': 'счетов',
      'exit_app': 'Выйти из приложения',
      'exit_confirm': 'Вы хотите закрыть приложение?',
      'exit': 'Выход',
      'bills': 'Счета',
      'vehicle_search': 'Поиск авто',
      'not_authenticated': 'Не авторизован',
      'network_error': 'Ошибка сети',
      'failed_to_load': 'Не удалось загрузить',
      'retry': 'Повторить',
      'save': 'Сохранить',
      'saving': 'Сохранение...',
      'delete': 'Удалить',
      'add': 'Добавить',
      'create': 'Создать',
      'date': 'Дата',
      'start': 'Начало',
      'end': 'Конец',
      'status': 'Статус',
      'description': 'Описание',
      'name': 'Имя',
      'phone': 'Телефон',
      'email': 'Email',
      'address': 'Адрес',
      'actions': 'Действия',
      'total': 'Всего',
      'balance': 'Баланс',
      'amount': 'Сумма',
      'all': 'Все',
      'available_schemes': 'Доступные схемы',
      'no_schemes_available': 'Нет доступных схем',
      'add_scheme': 'Добавить схему',
      'scheme_name': 'Название схемы',
      'scheme_type': 'Тип схемы',
      'paid_visits': 'Оплаченные визиты',
      'free_visits': 'Бесплатные визиты',
      'discount_percentage': 'Процент скидки',
      'voucher_no': '№ ваучера',
      'add_voucher': 'Добавить ваучер',
      'services': 'Услуги',
      'customer_types': 'Типы клиентов',
      'vehicle_types': 'Типы авто',
      'save_scheme': 'Сохранить схему',
      'scheme_created': 'Схема создана',
      'enter_scheme_details_error': 'Введите название и тип схемы',
      'booking_details': 'Детали бронирования',
      'customer_name': 'Имя клиента',
      'phone_number': 'Номер телефона',
      'vehicle_number': 'Номер авто',
      'select_service': 'Выберите услугу',
      'select_date': 'Выберите дату',
      'time_slot': 'Временной интервал',
      'booking_created': 'Бронирование создано',
      'booking_failed': 'Ошибка бронирования',
      'bookings_list': 'Список бронирований',
      'no_bookings_found': 'Бронирования не найдены',
      'receipts': 'Квитанции',
      'no_receipts_found': 'Квитанции не найдены',
      'collection': 'Сбор средств',
      'collect_payment': 'Принять оплату',
      'payment_collected': 'Оплата принята',
      'add_complaint': 'Подать жалобу',
      'subject': 'Тема',
      'complaint_created': 'Жалоба создана',
      'no_complaints_found': 'Жалобы не найдены',
      'add_new_customer': 'Добавить клиента',
      'edit_customer': 'Редактировать клиента',
      'customer_created': 'Клиент создан',
      'customer_updated': 'Клиент обновлен',
      'no_customers_found': 'Клиенты не найдены',
      'create_invoice': 'Создать счет',
      'invoice_details': 'Детали счета',
      'invoice_created': 'Счет создан',
      'invoice_number': 'Номер счета',
      'subtotal': 'Подытог',
      'tax': 'Налог',
      'grand_total': 'Итоговая сумма',
      'select_vehicle': 'Выберите авто',
      'select_scheme': 'Выберите схему',
      'job_details': 'Детали заказа',
      'revenue_report': 'Отчет по выручке',
      'jobs_report': 'Отчет по заказам',
      'sales_summary': 'Итоги продаж',
      'search_by_vehicle_number': 'Поиск по номеру авто',
      'vehicle_details': 'Детали авто',
      'msg_welcome': 'Здравствуйте, {customer}! 👋 Добро пожаловать в наш сервисный центр! Ваш автомобиль {vehicle} прибыл и находится в надёжных руках. Мы будем держать вас в курсе!',
      'msg_ready_alert': 'Здравствуйте, {customer}! 🎉 Отличная новость! Ваш автомобиль {vehicle} готов к получению. Пожалуйста, заберите его при первой возможности.',
      'msg_thanks': 'Уважаемый {customer}, 🙏 спасибо за доверие к нашему сервису для вашего автомобиля {vehicle}. Надеемся, вы остались довольны. Ждём вас снова!',
    },
    'zh': {
      'dashboard': '仪表板',
      'menu': '菜单',
      'new_job': '新建工单',
      'book_now': '立即预约',
      'bookings': '预约列表',
      'bill': '账单',
      'outstanding': '待结账款',
      'receipt': '收据',
      'customers': '客户',
      'schemes': '优惠方案',
      'reports': '报表中心',
      'vehicle': '车辆',
      'notifications': '消息通知',
      'language': '语言设置',
      'complaints': '投诉反馈',
      'logout': '退出登录',
      'welcome_back': '欢迎回来',
      'add_extras': '添加附加项',
      'services_subtotal': '服务小计',
      'scheme_discount': '方案折扣',
      'manual_discount': '手动折扣',
      'bill_summary': '账单明细',
      'amount_collected': '已收金额',
      'taxes': '税费',
      'quick_menu': '快捷菜单',
      'login': '登录',
      'username_or_email': '用户名或邮箱',
      'enter_username_or_email': '请输入用户名或邮箱',
      'password': '密码',
      'enter_password': '请输入密码',
      'remember_me': '记住密码',
      'forgot_password': '忘记密码？',
      'sign_in': '登录',
      'logout_confirm': '确定要退出登录吗？',
      'cancel': '取消',
      'branch_admin': '分店管理员',
      'company_admin': '公司管理员',
      'recent_invoices': '近期发票',
      'select_language': '选择语言',
      'admin_login': '管理员登录',
      'username': '用户名',
      'please_enter_credentials': '请输入用户名和密码',
      'todays_summary': '今日汇总',
      'overview': '概览',
      'todays_recent_jobs': '今日最新工单',
      'revenue': '营业额',
      'collected': '已实收',
      'jobs': '工单数',
      'due': '应付/欠款',
      'paid': '已付',
      'invoices_count': '张发票',
      'exit_app': '退出应用',
      'exit_confirm': '确定要关闭应用吗？',
      'exit': '退出',
      'bills': '账单列表',
      'vehicle_search': '车辆查询',
      'not_authenticated': '未授权/未登录',
      'network_error': '网络连接错误',
      'failed_to_load': '加载失败',
      'retry': '重试',
      'save': '保存',
      'saving': '保存中...',
      'delete': '删除',
      'add': '添加',
      'create': '创建',
      'date': '日期',
      'start': '开始',
      'end': '结束',
      'status': '状态',
      'description': '描述说明',
      'name': '姓名',
      'phone': '电话',
      'email': '邮箱',
      'address': '地址',
      'actions': '操作',
      'total': '合计',
      'balance': '余额/欠款',
      'amount': '金额',
      'all': '全部',
      'available_schemes': '可用优惠方案',
      'no_schemes_available': '暂无可用方案',
      'add_scheme': '添加方案',
      'scheme_name': '方案名称',
      'scheme_type': '方案类型',
      'paid_visits': '付费次数',
      'free_visits': '免费次数',
      'discount_percentage': '折扣比例',
      'voucher_no': '代金券号',
      'add_voucher': '添加代金券',
      'services': '服务项目',
      'customer_types': '客户类型',
      'vehicle_types': '车辆类型',
      'save_scheme': '保存方案',
      'scheme_created': '方案创建成功',
      'enter_scheme_details_error': '请输入方案名称及类型',
      'booking_details': '预约详情',
      'customer_name': '客户姓名',
      'phone_number': '手机号码',
      'vehicle_number': '车牌号码',
      'select_service': '选择服务',
      'select_date': '选择日期',
      'time_slot': '时间段',
      'booking_created': '预约创建成功',
      'booking_failed': '预约失败',
      'bookings_list': '预约列表',
      'no_bookings_found': '未找到预约记录',
      'receipts': '收据列表',
      'no_receipts_found': '未找到收据记录',
      'collection': '收款管理',
      'collect_payment': '收取款项',
      'payment_collected': '收款成功',
      'add_complaint': '新建投诉',
      'subject': '投诉主题',
      'complaint_created': '投诉提交成功',
      'no_complaints_found': '未找到投诉记录',
      'add_new_customer': '添加新客户',
      'edit_customer': '编辑客户信息',
      'customer_created': '客户创建成功',
      'customer_updated': '客户信息已更新',
      'no_customers_found': '未找到客户',
      'create_invoice': '开具发票/账单',
      'invoice_details': '发票明细',
      'invoice_created': '发票开具成功',
      'invoice_number': '发票编号',
      'subtotal': '小计',
      'tax': '税额',
      'grand_total': '总计',
      'select_vehicle': '选择车辆',
      'select_scheme': '选择方案',
      'job_details': '工单详情',
      'revenue_report': '营收报表',
      'jobs_report': '工单报表',
      'sales_summary': '销售汇总',
      'search_by_vehicle_number': '按车牌号搜索',
      'vehicle_details': '车辆详情',
      'msg_welcome': '您好，{customer}！👋 欢迎来到我们的服务中心。您的车辆 {vehicle} 已安全到达，正在专业人员的照料中。我们会随时通知您！',
      'msg_ready_alert': '您好，{customer}！🎉 好消息！您的车辆 {vehicle} 已准备好取车。请尽快来取。',
      'msg_thanks': '亲爱的 {customer}，🙏 感谢您将车辆 {vehicle} 交给我们服务。希望您有美好的体验。期待再次为您服务！',
    },
    'vi': {
      'dashboard': 'Bảng điều khiển',
      'menu': 'Danh mục',
      'new_job': 'Công việc mới',
      'book_now': 'Đặt lịch ngay',
      'bookings': 'Danh sách đặt lịch',
      'bill': 'Hóa đơn',
      'outstanding': 'Công nợ',
      'receipt': 'Biên nhận',
      'customers': 'Khách hàng',
      'schemes': 'Gói dịch vụ',
      'reports': 'Báo cáo',
      'vehicle': 'Phương tiện',
      'notifications': 'Thông báo',
      'language': 'Ngôn ngữ',
      'complaints': 'Khiếu nại',
      'logout': 'Đăng xuất',
      'welcome_back': 'Chào mừng trở lại',
      'add_extras': 'Thêm dịch vụ phụ',
      'services_subtotal': 'Tổng dịch vụ',
      'scheme_discount': 'Giảm giá theo gói',
      'manual_discount': 'Giảm giá thủ công',
      'bill_summary': 'Tóm tắt hóa đơn',
      'amount_collected': 'Số tiền đã thu',
      'taxes': 'Thuế',
      'quick_menu': 'Menu nhanh',
      'login': 'Đăng nhập',
      'username_or_email': 'Tên đăng nhập hoặc Email',
      'enter_username_or_email': 'Nhập tên đăng nhập hoặc Email',
      'password': 'Mật khẩu',
      'enter_password': 'Nhập mật khẩu',
      'remember_me': 'Ghi nhớ đăng nhập',
      'forgot_password': 'Quên mật khẩu?',
      'sign_in': 'Đăng nhập',
      'logout_confirm': 'Bạn có chắc chắn muốn đăng xuất?',
      'cancel': 'Hủy',
      'branch_admin': 'Quản lý chi nhánh',
      'company_admin': 'Quản lý công ty',
      'recent_invoices': 'Hóa đơn gần đây',
      'select_language': 'Chọn ngôn ngữ',
      'admin_login': 'Đăng nhập Quản trị',
      'username': 'Tên đăng nhập',
      'please_enter_credentials': 'Vui lòng nhập tên đăng nhập và mật khẩu',
      'todays_summary': 'Tóm tắt hôm nay',
      'overview': 'Tổng quan',
      'todays_recent_jobs': 'Công việc gần đây hôm nay',
      'revenue': 'Doanh thu',
      'collected': 'Đã thu',
      'jobs': 'Công việc',
      'due': 'Còn nợ',
      'paid': 'Đã thanh toán',
      'invoices_count': 'hóa đơn',
      'exit_app': 'Thoát ứng dụng',
      'exit_confirm': 'Bạn có muốn đóng ứng dụng không?',
      'exit': 'Thoát',
      'bills': 'Hóa đơn',
      'vehicle_search': 'Tìm kiếm xe',
      'not_authenticated': 'Chưa xác thực',
      'network_error': 'Lỗi kết nối mạng',
      'failed_to_load': 'Tải thất bại',
      'retry': 'Thử lại',
      'save': 'Lưu',
      'saving': 'Đang lưu...',
      'delete': 'Xóa',
      'add': 'Thêm',
      'create': 'Tạo',
      'date': 'Ngày',
      'start': 'Bắt đầu',
      'end': 'Kết thúc',
      'status': 'Trạng thái',
      'description': 'Mô tả',
      'name': 'Tên',
      'phone': 'Số điện thoại',
      'email': 'Email',
      'address': 'Địa chỉ',
      'actions': 'Thao tác',
      'total': 'Tổng cộng',
      'balance': 'Số dư/Còn lại',
      'amount': 'Số tiền',
      'all': 'Tất cả',
      'available_schemes': 'Gói ưu đãi khả dụng',
      'no_schemes_available': 'Không có gói ưu đãi nào',
      'add_scheme': 'Thêm gói',
      'scheme_name': 'Tên gói',
      'scheme_type': 'Loại gói',
      'paid_visits': 'Số lần trả phí',
      'free_visits': 'Số lần miễn phí',
      'discount_percentage': 'Phần trăm giảm giá',
      'voucher_no': 'Mã phiếu',
      'add_voucher': 'Thêm phiếu',
      'services': 'Dịch vụ',
      'customer_types': 'Loại khách hàng',
      'vehicle_types': 'Loại xe',
      'save_scheme': 'Lưu gói',
      'scheme_created': 'Đã tạo gói',
      'enter_scheme_details_error': 'Nhập tên và loại gói',
      'booking_details': 'Chi tiết đặt lịch',
      'customer_name': 'Tên khách hàng',
      'phone_number': 'Số điện thoại',
      'vehicle_number': 'Biển số xe',
      'select_service': 'Chọn dịch vụ',
      'select_date': 'Chọn ngày',
      'time_slot': 'Khung giờ',
      'booking_created': 'Đặt lịch thành công',
      'booking_failed': 'Đặt lịch thất bại',
      'bookings_list': 'Danh sách đặt lịch',
      'no_bookings_found': 'Không tìm thấy đặt lịch',
      'receipts': 'Biên nhận',
      'no_receipts_found': 'Không tìm thấy biên nhận',
      'collection': 'Thu tiền',
      'collect_payment': 'Thu thanh toán',
      'payment_collected': 'Đã thu thanh toán',
      'add_complaint': 'Thêm khiếu nại',
      'subject': 'Chủ đề',
      'complaint_created': 'Đã gửi khiếu nại',
      'no_complaints_found': 'Không tìm thấy khiếu nại',
      'add_new_customer': 'Thêm khách hàng mới',
      'edit_customer': 'Sửa thông tin khách hàng',
      'customer_created': 'Đã tạo khách hàng',
      'customer_updated': 'Đã cập nhật khách hàng',
      'no_customers_found': 'Không tìm thấy khách hàng',
      'create_invoice': 'Tạo hóa đơn',
      'invoice_details': 'Chi tiết hóa đơn',
      'invoice_created': 'Đã tạo hóa đơn',
      'invoice_number': 'Số hóa đơn',
      'subtotal': 'Tạm tính',
      'tax': 'Thuế',
      'grand_total': 'Tổng tiền',
      'select_vehicle': 'Chọn xe',
      'select_scheme': 'Chọn gói',
      'job_details': 'Chi tiết công việc',
      'revenue_report': 'Báo cáo doanh thu',
      'jobs_report': 'Báo cáo công việc',
      'sales_summary': 'Tóm tắt doanh số',
      'search_by_vehicle_number': 'Tìm theo biển số xe',
      'vehicle_details': 'Chi tiết xe',
      'msg_welcome': 'Xin chào {customer}! 👋 Chào mừng đến với trung tâm dịch vụ của chúng tôi. Xe {vehicle} của bạn đã đến an toàn và đang được chuyên gia chăm sóc. Chúng tôi sẽ cập nhật cho bạn!',
      'msg_ready_alert': 'Xin chào {customer}! 🎉 Tin vui! Xe {vehicle} của bạn đã sẵn sàng để lấy. Vui lòng đến nhận sớm.',
      'msg_thanks': 'Kính gửi {customer}, 🙏 Cảm ơn bạn đã tin tưởng chúng tôi với xe {vehicle}. Hy vọng bạn có trải nghiệm tuyệt vời. Hẹn gặp lại!',
    },
    'th': {
      'dashboard': 'แผงควบคุม',
      'menu': 'เมนู',
      'new_job': 'งานใหม่',
      'book_now': 'จองทันที',
      'bookings': 'รายการจอง',
      'bill': 'บิล',
      'outstanding': 'ยอดค้างชำระ',
      'receipt': 'ใบเสร็จรับเงิน',
      'customers': 'ลูกค้า',
      'schemes': 'แพ็กเกจ/โปรโมชัน',
      'reports': 'รายงาน',
      'vehicle': 'ยานพาหนะ',
      'notifications': 'การแจ้งเตือน',
      'language': 'ภาษา',
      'complaints': 'ข้อร้องเรียน',
      'logout': 'ออกจากระบบ',
      'welcome_back': 'ยินดีต้อนรับกลับ',
      'add_extras': 'เพิ่มบริการเสริม',
      'services_subtotal': 'รวมบริการย่อย',
      'scheme_discount': 'ส่วนลดแพ็กเกจ',
      'manual_discount': 'ส่วนลดกำหนดเอง',
      'bill_summary': 'สรุปบิล',
      'amount_collected': 'ยอดเงินที่ได้รับ',
      'taxes': 'ภาษี',
      'quick_menu': 'เมนูด่วน',
      'login': 'เข้าสู่ระบบ',
      'username_or_email': 'ชื่อผู้ใช้หรืออีเมล',
      'enter_username_or_email': 'กรอกชื่อผู้ใช้หรืออีเมล',
      'password': 'รหัสผ่าน',
      'enter_password': 'กรอกรหัสผ่าน',
      'remember_me': 'จดจำฉัน',
      'forgot_password': 'ลืมรหัสผ่าน?',
      'sign_in': 'ลงชื่อเข้าใช้',
      'logout_confirm': 'คุณแน่ใจหรือไม่ว่าต้องการออกจากระบบ?',
      'cancel': 'ยกเลิก',
      'branch_admin': 'ผู้ดูแลสาขา',
      'company_admin': 'ผู้ดูแลบริษัท',
      'recent_invoices': 'ใบแจ้งหนี้ล่าสุด',
      'select_language': 'เลือกภาษา',
      'admin_login': 'เข้าสู่ระบบผู้ดูแล',
      'username': 'ชื่อผู้ใช้',
      'please_enter_credentials': 'กรุณากรอกชื่อผู้ใช้และรหัสผ่าน',
      'todays_summary': 'สรุปวันนี้',
      'overview': 'ภาพรวม',
      'todays_recent_jobs': 'งานล่าสุดวันนี้',
      'revenue': 'รายได้',
      'collected': 'รับแล้ว',
      'jobs': 'จำนวนงาน',
      'due': 'ค้างชำระ',
      'paid': 'ชำระแล้ว',
      'invoices_count': 'ใบแจ้งหนี้',
      'exit_app': 'ออกจากแอป',
      'exit_confirm': 'คุณต้องการปิดแอปพลิเคชันหรือไม่?',
      'exit': 'ออก',
      'bills': 'บิลทั้งหมด',
      'vehicle_search': 'ค้นหารถ',
      'not_authenticated': 'ยังไม่ได้ยืนยันตัวตน',
      'network_error': 'ข้อผิดพลาดเครือข่าย',
      'failed_to_load': 'โหลดข้อมูลไม่สำเร็จ',
      'retry': 'ลองใหม่',
      'save': 'บันทึก',
      'saving': 'กำลังบันทึก...',
      'delete': 'ลบ',
      'add': 'เพิ่ม',
      'create': 'สร้าง',
      'date': 'วันที่',
      'start': 'เริ่ม',
      'end': 'สิ้นสุด',
      'status': 'สถานะ',
      'description': 'รายละเอียด',
      'name': 'ชื่อ',
      'phone': 'โทรศัพท์',
      'email': 'อีเมล',
      'address': 'ที่อยู่',
      'actions': 'การดำเนินการ',
      'total': 'รวมทั้งสิ้น',
      'balance': 'ยอดคงเหลือ',
      'amount': 'จำนวนเงิน',
      'all': 'ทั้งหมด',
      'available_schemes': 'แพ็กเกจที่มีอยู่',
      'no_schemes_available': 'ไม่มีแพ็กเกจ',
      'add_scheme': 'เพิ่มแพ็กเกจ',
      'scheme_name': 'ชื่อแพ็กเกจ',
      'scheme_type': 'ประเภทแพ็กเกจ',
      'paid_visits': 'จำนวนครั้งที่จ่าย',
      'free_visits': 'จำนวนครั้งฟรี',
      'discount_percentage': 'เปอร์เซ็นต์ส่วนลด',
      'voucher_no': 'เลขที่วอชเชอร์',
      'add_voucher': 'เพิ่มวอชเชอร์',
      'services': 'บริการ',
      'customer_types': 'ประเภทลูกค้า',
      'vehicle_types': 'ประเภทยานพาหนะ',
      'save_scheme': 'บันทึกแพ็กเกจ',
      'scheme_created': 'สร้างแพ็กเกจสำเร็จ',
      'enter_scheme_details_error': 'กรุณากรอกชื่อและประเภทแพ็กเกจ',
      'booking_details': 'รายละเอียดการจอง',
      'customer_name': 'ชื่อลูกค้า',
      'phone_number': 'หมายเลขโทรศัพท์',
      'vehicle_number': 'ทะเบียนรถ',
      'select_service': 'เลือกบริการ',
      'select_date': 'เลือกวันที่',
      'time_slot': 'ช่วงเวลา',
      'booking_created': 'สร้างการจองสำเร็จ',
      'booking_failed': 'การจองไม่สำเร็จ',
      'bookings_list': 'รายการจองทั้งหมด',
      'no_bookings_found': 'ไม่พบรายการจอง',
      'receipts': 'ใบเสร็จรับเงิน',
      'no_receipts_found': 'ไม่พบใบเสร็จ',
      'collection': 'การรับชำระเงิน',
      'collect_payment': 'รับชำระเงิน',
      'payment_collected': 'รับชำระเงินเรียบร้อย',
      'add_complaint': 'เพิ่มข้อร้องเรียน',
      'subject': 'หัวข้อ',
      'complaint_created': 'ส่งข้อร้องเรียนเรียบร้อย',
      'no_complaints_found': 'ไม่พบข้อร้องเรียน',
      'add_new_customer': 'เพิ่มลูกค้าใหม่',
      'edit_customer': 'แก้ไขข้อมูลลูกค้า',
      'customer_created': 'เพิ่มลูกค้าสำเร็จ',
      'customer_updated': 'อัปเดตข้อมูลลูกค้าสำเร็จ',
      'no_customers_found': 'ไม่พบลูกค้า',
      'create_invoice': 'สร้างใบแจ้งหนี้',
      'invoice_details': 'รายละเอียดใบแจ้งหนี้',
      'invoice_created': 'สร้างใบแจ้งหนี้สำเร็จ',
      'invoice_number': 'เลขที่ใบแจ้งหนี้',
      'subtotal': 'ยอดรวมย่อย',
      'tax': 'ภาษี',
      'grand_total': 'ยอดรวมสุทธิ',
      'select_vehicle': 'เลือกรถ',
      'select_scheme': 'เลือกแพ็กเกจ',
      'job_details': 'รายละเอียดงาน',
      'revenue_report': 'รายงานรายได้',
      'jobs_report': 'รายงานจำนวนงาน',
      'sales_summary': 'สรุปยอดขาย',
      'search_by_vehicle_number': 'ค้นหาด้วยทะเบียนรถ',
      'vehicle_details': 'รายละเอียดรถ',
      'msg_welcome': 'สวัสดีครับ {customer}! 👋 ยินดีต้อนรับสู่ศูนย์บริการของเรา รถ {vehicle} ของคุณมาถึงแล้วและอยู่ในมือของผู้เชี่ยวชาญ เราจะแจ้งให้คุณทราบ!',
      'msg_ready_alert': 'สวัสดีครับ {customer}! 🎉 ข่าวดี! รถ {vehicle} ของคุณพร้อมรับแล้ว กรุณามารับในเร็วๆ นี้',
      'msg_thanks': 'เรียน {customer}, 🙏 ขอบคุณที่ไว้วางใจเราดูแลรถ {vehicle} ของคุณ หวังว่าคุณจะได้รับประสบการณ์ที่ดี พบกันใหม่นะครับ!',
    },
  };
}

// BuildContext extension shortcut
extension LanguageExtension on BuildContext {
  String translate(String key) {
    try {
      return Provider.of<LanguageProvider>(this).translate(key);
    } catch (_) {
      return Provider.of<LanguageProvider>(this, listen: false).translate(key);
    }
  }

  String tr(String text) {
    try {
      return Provider.of<LanguageProvider>(this).translateDynamic(text);
    } catch (_) {
      return Provider.of<LanguageProvider>(this, listen: false).translateDynamic(text);
    }
  }
}
